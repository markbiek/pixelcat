...
@Test func topRowMapsToTheTopOfTheImage() throws {
    let sheet = try SpriteSheet(manifest: makeManifest(), sheetSize: sheetSize)
...
@Test func rejectsASheetTooNarrowForItsFrames() {
    ...
    #expect(throws: SpriteSheetError.self) {
        try SpriteSheet(manifest: makeManifest(), sheetSize: CGSize(width: 128, height: 96))
    }
}
...
@Test func rejectsASheetTooShortForItsRows() {
    ...
    #expect(throws: SpriteSheetError.self) {
        try SpriteSheet(manifest: makeManifest(), sheetSize: CGSize(width: 192, height: 64))
    }
}
